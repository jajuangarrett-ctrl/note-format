...
export class CaptureModal extends Modal {
  ...
  private youtubeInput: HTMLInputElement | null = null;
  private youtubeButton: ButtonComponent | null = null;
  private recorder: VoiceRecorder | null = null;
  private recording = false;
  private busy = false;
  private fetchingYouTube = false;
  ...
  async onOpen() {
    ...
    new Setting(contentEl)
      .setName("YouTube URL")
      .setDesc("Paste a YouTube URL and tap Fetch to pull the video's captions into the transcript field below. Tap Save to format and store the note like any other transcript.")
      .addText((t) => {
        this.youtubeInput = t.inputEl;
        t.inputEl.type = "url";
        t.inputEl.style.width = "100%";
        t.setPlaceholder("https://www.youtube.com/watch?v=...");
      })
      .addButton((b) => {
        this.youtubeButton = b;
        b.setButtonText("Fetch").onClick(() => this.fetchYouTube());
      });
  ...
  private async fetchYouTube() {
    if (this.busy || this.fetchingYouTube || !this.youtubeButton) return;
    const url = (this.youtubeInput?.value || "").trim();
    if (!url) {
      new Notice("Paste a YouTube URL first.");
      return;
    }

    this.fetchingYouTube = true;
    this.busy = true;
    this.youtubeButton.setDisabled(true);
    this.youtubeButton.setButtonText("Fetching...");
    this.setSaveButtonsDisabled(true);

    try {
      const result = await fetchYouTubeTranscript(url);
      this.text = mergeTranscript(this.text, result.text);
      if (this.textArea) {
        this.textArea.value = this.text;
        this.textArea.focus();
      }
      const label = result.title ? `"${result.title}"` : "YouTube video";
      const flavor = result.isAutoGenerated ? " (auto-generated captions)" : "";
      new Notice(`Pulled transcript from ${label}${flavor}.`);
    } catch (e) {
      new Notice(`YouTube fetch failed: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      this.fetchingYouTube = false;
      this.busy = false;
      if (this.youtubeButton) {
        this.youtubeButton.setDisabled(false);
        this.youtubeButton.setButtonText("Fetch");
      }
